...
namespace AnnoMapEditor.Mods.Serialization
{
    ...
    public class AnnoEditorLevel
    {
        ...
        public AnnoEditorLevel(int dimensions)
            : this(dimensions, fileVersion: 3, useHexSequentialIds: false) { }

        /// <summary>
        /// Anno 117 uses FileVersion=4 and sequential hex IDs for chunks (matches Taludas mods).
        /// </summary>
        public static AnnoEditorLevel CreateForAnno117(int dimensions)
            => new(dimensions, fileVersion: 4, useHexSequentialIds: true);

        private AnnoEditorLevel(int dimensions, int fileVersion, bool useHexSequentialIds)
        {
