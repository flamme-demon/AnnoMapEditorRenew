...
namespace AnnoMapEditor.MapTemplates.Serializing
{
    public class MapTemplateReader
    {
        public async Task<MapTemplate> FromDataArchiveAsync(string a7tinfoPath)
        {
            ...
            // Anno 117: assets.xml stores TemplateFilename ending in .a7t (Anvil terrain),
            // but the BBDom-V3 template structure lives next to it as .a7tinfo.
            if (Path.GetExtension(a7tinfoPath).Equals(".a7t", StringComparison.OrdinalIgnoreCase))
                a7tinfoPath = Path.ChangeExtension(a7tinfoPath, ".a7tinfo");
        ...
        public async Task<MapTemplate> FromBinaryStreamAsync(SessionAsset session, Stream a7tinfoStream)
        {
            ...
            var template = new MapTemplate(doc, session);
            template.SourceVersion = FileDBSerializer.LastReadVersion;
            return template;
        }
